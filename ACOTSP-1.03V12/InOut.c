...
void write_registro_antes(long int *tour, long int length){
  
  if (registro){
      fprintf(registro," RANK = %i envía tour %ld-%ld-%ld-...-%ld-%ld-%ld -> Length: %ld \n", rank, tour[0], tour[1], tour[2], tour[n-2], tour[n-1], tour[n], length);
      fflush(registro);  
  }
...
void write_registro_despues(long int *tour, long int length){
  ...
  if (registro){
      fprintf(registro," RANK = %i recibe tour %ld-%ld-%ld-...-%ld-%ld-%ld -> Length: %ld \n", rank, tour[0], tour[1], tour[2], tour[n-2], tour[n-1], tour[n], length);
      fflush(registro); 
...
void write_best_global_tour(long int length){
  if(registro){
     fprintf(registro," RANK = %i best global tour %ld\n", rank, length);
  }
}
