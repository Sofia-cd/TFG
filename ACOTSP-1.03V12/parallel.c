...
/*void BcastBestSolutionToColonies ( void )
{
  ...
  for(i=0; i<numprocs; i++){
     ...
     if (i == rank){
        memcpy(tour_best_ant, best_so_far_ant->tour, n+1);
        write_registro_antes(tour_best_ant);
     }
     ...
     if (rank != i ){
        ...
        write_registro_despues(tour_best_ant);
       ...
       /* if ( eas_flag && distanceOwn > distanceForeing){
           ...
           if (global_best_tour > distanceForeing){
           	global_best_tour = distanceForeing;best_so_far_ant->tour_length = INFTY;
...
void startCommColonies ( void )
{
	...
	for(i=0;i<numprocs;i++){
	        if(i != rank)  
			MPI_Irecv(foreign_tours[i].foreign_tour, n+1, MPI_LONG, i, 2000, MPI_COMM_WORLD, &foreign_tours[i].request);
        }
...
void sendBestSolutionToColonies ( void )
{
	...
	int i,numBytes;
	MPI_Request request; /* IDENTIFICADOR PARA UNA OPERACION SIN BLOQUEO */
...
void listenColonies( void )
{
	...
	/*if( distanceOwn < INFTY){
		numBytes= 8*(n+1);
       	...
       	if(global_best_tour >  distanceOwn){  
	...
	for(i = 0; i < numprocs; i++){
		...
		if( rank != i){
			...
			while(ready){
				MPI_Test(&foreign_tours[i].request, &ready, MPI_STATUS_IGNORE);
				...
				if(ready){
					...
					if ( eas_flag && distanceOwn > distanceForeing){
						...
						foreign_solution_update_pheromone_weighted(foreign_tours[i].foreign_tour, elitist_ants);
