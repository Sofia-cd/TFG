...
long int global_best_tour; /* the best tour of all the colonies */

buffer_tours_struct *foreign_tours;
long int *own_tour; /* own tours buffer */ 
...
*/
{
    ...
    if((foreign_tours = malloc(sizeof( buffer_tours_struct ) * numprocs +
		     sizeof(buffer_tours_struct *) * numprocs	 )) == NULL){
	printf("Out of memory, exit.");
	exit(1);
    }
    for ( i = 0 ; i < numprocs ; i++ ) {
	foreign_tours[i].foreign_tour = calloc(n+1, sizeof(long int));
    }
    
    if ((own_tour = calloc(n+1, sizeof(long int))) == NULL) {
	printf("Out of memory, exit.");
	exit(1);
    }
