...
FILE *report, *comp_report, *stat_report, *timeMeasurements_report, *total_results_report, *registro;
...
*/
{
  ...
  if (!quiet_flag) {
      sprintf(temp_buffer,"best.%s.nThreads%d.NPROC%d",instance.name,omp_get_max_threads(),rank);
      TRACE ( printf("%s\n",temp_buffer); )
          report = fopen(temp_buffer, "w");
      sprintf(temp_buffer,"cmp.%s.nThreads%d.NPROC%d",instance.name,omp_get_max_threads(),rank);
      TRACE ( printf("%s\n",temp_buffer); )
          comp_report = fopen(temp_buffer, "w");
      sprintf(temp_buffer,"stat.%s.nThreads%d.NPROC%d",instance.name,omp_get_max_threads(),rank);
      TRACE ( printf("%s\n",temp_buffer); )
          stat_report = fopen(temp_buffer, "w");
      sprintf(temp_buffer,"timeMeasurements.%s.nThreads%d.NPROC%d",instance.name,omp_get_max_threads(),rank);
      TRACE ( printf("%s\n",temp_buffer); )
          timeMeasurements_report = fopen(temp_buffer, "w");
      if (rank == 0){
      sprintf(temp_buffer,"total_results_report.%s.nThreads%d",instance.name,omp_get_max_threads());
      TRACE ( printf("%s\n",temp_buffer); )
      total_results_report = fopen(temp_buffer, "w");     
      }
      sprintf(temp_buffer,"registro.%s.nThreads%d.NPROC%d",instance.name,omp_get_max_threads(),rank);
      TRACE ( printf("%s\n",temp_buffer); )
      registro = fopen(temp_buffer, "w");
...
void write_totalResults(int rankSource, long int best, long int iterations, double b_fact, double time){
  if (total_results_report)
      fprintf(total_results_report," RANK = %i Best: %ld\t Iterations: %6ld\t B-Fac %.5f\t Time %.2f\t Tot.time %.2f\n", rankSource, best, iterations, b_fact, time, elapsed_time( REAL ));
}

void imprimir_resultados_parciales (){
  fflush(total_results_report);
}

void write_registro_antes(long int *tour, long int length, long int lengthOrig){
  
  if (registro){
      fprintf(registro,"TRY = %ld  RANK = %i envía tour %ld-%ld-%ld-...-%ld-%ld-%ld LENGTH = %ld  LENGTH ORIGINAL = %ld\n", n_try, rank, tour[0], tour[1], tour[2], tour[n-2], tour[n-1], tour[n], length, lengthOrig);
      fflush(registro);  
  }
       
}

void write_registro_despues(long int *tour, long int length){
  if (registro){
      fprintf(registro," TRY = %ld RANK = %i recibe tour %ld-%ld-%ld-...-%ld-%ld-%ld LENGTH = %ld\n", n_try, rank, tour[0], tour[1], tour[2], tour[n-2], tour[n-1], tour[n], length);
      fflush(registro); 
...
void write_best_global_tour(long int length){
  if(registro){
     fprintf(registro," TRY = %ld RANK = %i best global tour %ld\n", n_try, rank, length);
  }
}

void write_registro_best_global(long int length){
  if (registro){
  	fprintf(registro," Valor de best global (Condicion exit) tour %ld try %ld \n", length, n_try);
  }
}
